...
3. **`src/registry.ts`** — 依赖的唯一真源。`REGISTRY` 把每个 import key 映射到 `{ import 语句, pkg 包名, version 版本, byFramework? 按框架覆盖 }`（对象定义顺序即 import 输出顺序）；`PEER` 描述运行必需但不被直接 import 的 peer 依赖（`always` 恒需，如 `typescript`；`perImport` 随某 import key 选中才需，如 `yml → yaml-eslint-parser`），同样内联版本；`versionFor(dep, framework)` 取 `byFramework[framework] ?? version`（如 eslint 在 next 锁 9.x、vite 用 10.x）。**版本不再放外部 json，全部内联于此。**
...
7. **`src/paths.ts`** — 运行时定位数据文件。无论开发态（`src/`）还是打包后（`dist/index.js`），`pkgRoot` 都是当前文件目录的上一级，`srcDir = <pkgRoot>/src`。因此 `src/rules` **必须随包发布**（见 `package.json#files`）——tsup 只打包代码，规则数据是运行时读取的（依赖版本已内联进代码，不再有运行时读取的 json）。
...
- 依赖版本来自 `src/registry.ts`（`REGISTRY` / `PEER` 内联，按 `versionFor` 解析框架差异），不再读外部文件。

### 改动指南

- **改某条 lint 规则**：编辑 `src/rules/*.rule.jsonc`，不要动 `build.ts`。
- **新增一个插件/import**：在 `registry.ts` 的 `REGISTRY` 里登记一条（含 import 语句、pkg、version；如版本随框架不同则加 `byFramework`），如有 peer 依赖加进 `PEER`，再在 `build.ts` 的 `collectImportKeys` 与相应 render 块里接入。
